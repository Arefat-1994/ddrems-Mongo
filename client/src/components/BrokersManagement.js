...
const BrokersManagement = ({ onBack }) => {
  ...
  const fetchBrokers = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${window.API_URL}/brokers`);
  ...
  const handleAddBroker = async (e) => {
    ...
    try {
      // Create user account
      const userResponse = await axios.post(`${window.API_URL}/auth/register`, {
        name: addForm.name,
        email: addForm.email,
        phone: addForm.phone,
      ...
      await axios.post(`${window.API_URL}/brokers`, {
