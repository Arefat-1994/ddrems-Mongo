...
const MyBookings = ({ user, setCurrentPage }) => {
  ...
  const handleCancel = async (bookingId) => {
    ...
    try {
      await axios.put(`${window.API_URL}/broker-bookings/${bookingId}/cancel`);
      fetchBookings();
    } catch (error) {
  ...
  const handleDelete = async (bookingId) => {
    ...
    try {
      await axios.delete(`${window.API_URL}/broker-bookings/${bookingId}`);
      fetchBookings();
    } catch (error) {
  ...
  const handleViewProperty = async (propertyId) => {
    ...
    try {
      const res = await axios.get(`${window.API_URL}/properties/${propertyId}`);
