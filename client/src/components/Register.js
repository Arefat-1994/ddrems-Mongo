...
const Register = ({ onBackToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/auth/register`, {
        name: formData.name,
