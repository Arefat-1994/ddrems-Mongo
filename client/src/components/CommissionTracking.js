  ...
  const fetchCommissionData = useCallback(async () => {
    ...
    try {
      ...
      const [commRes, summaryRes, engRes] = await Promise.all([
        axios.get(`${window.API_URL}/commissions/broker/${user.id}`).catch(() => ({ data: [] })),
        axios.get(`${window.API_URL}/commissions/broker/${user.id}/summary`).catch(() => ({ data: {} })),
        axios.get(`${window.API_URL}/commissions/broker/${user.id}/engagements`).catch(() => ({ data: { engagements: [] } }))
      ]);
