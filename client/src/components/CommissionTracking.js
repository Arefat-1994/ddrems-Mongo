  ...
  const fetchCommissionData = useCallback(async () => {
    ...
    try {
      ...
      const [commRes, summaryRes, engRes] = await Promise.all([
        axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/commissions/broker/${user.id}`).catch(() => ({ data: [] })),
        axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/commissions/broker/${user.id}/summary`).catch(() => ({ data: {} })),
        axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/commissions/broker/${user.id}/engagements`).catch(() => ({ data: { engagements: [] } }))
      ]);
