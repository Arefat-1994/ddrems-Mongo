...
const SettingsPage = ({ user, onLogout }) => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      // Fetch preferences
      try {
        const prefsRes = await axios.get(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/user-settings/${user.id}`);
        if (prefsRes.data) {
      ...
      // Fetch 2FA settings
      try {
        const twoFARes = await axios.get(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/user-settings/${user.id}/two-factor`);
        if (twoFARes.data) {
  ...
  const handleSave = async () => {
    try {
      ...
      // Save preferences
      await axios.post(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/user-settings/${user.id}`, {
        theme,
        primaryColor,
        accentColor,
        backgroundColor,
      ...
      // Save 2FA settings
      await axios.post(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/user-settings/${user.id}/two-factor`, {
        twoFactorEnabled,
        twoFactorMethod
      });
