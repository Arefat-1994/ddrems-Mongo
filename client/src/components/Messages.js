...
const Messages = ({ user, onLogout, onSettingsClick }) => {
  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get(`${window.API_URL}/messages/user/${user.id}?userId=${user.id}`);
  ...
  const fetchUnreadCount = async () => {
    try {
      const response = await axios.get(`${window.API_URL}/messages/unread/${user.id}?userId=${user.id}`);
  ...
  const fetchMessageThread = async (messageId) => {
    try {
      const response = await axios.get(`${window.API_URL}/messages/${messageId}/thread?userId=${user.id}`);
      if (response.data.success !== false) {
  ...
  const handleMessageClick = async (message) => {
    ...
    if (!message.is_read) {
      try {
        await axios.put(`${window.API_URL}/messages/read/${message.id}?userId=${user.id}`);
  ...
  const handleMarkAllRead = async () => {
    try {
      await axios.put(`${window.API_URL}/messages/read-all/${user.id}?userId=${user.id}`);
  ...
  const handleDeleteMessage = async (messageId) => {
    if (window.confirm('Are you sure you want to delete this message?')) {
      try {
        await axios.delete(`${window.API_URL}/messages/${messageId}?userId=${user.id}`);
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${window.API_URL}/messages/${selectedMessage.id}?userId=${user.id}`, editForm);
      setMessages(messages.map(m => 
  ...
  const handleReplySubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${window.API_URL}/messages/${selectedMessage.id}/reply`, {
