...
const Dashboard = ({ user, onLogout, setCurrentPage, setViewMapPropertyId, onSettingsClick }) => {
  ...
  const fetchUnreadMessages = async () => {
    try {
      const response = await axios.get(`http://${window.location.hostname}:5000/api/messages/unread/${user.id}`);
      setUnreadMessages(response.data.count || 0);
      
      // Also fetch recent notifications
      const notifResponse = await axios.get(`http://${window.location.hostname}:5000/api/messages/notifications/${user.id}`);
  ...
  const fetchDashboardData = async () => {
    try {
      // 1. Fetch main stats
      try {
        const statsRes = await axios.get(`http://${window.location.hostname}:5000/api/dashboard/stats`);
        setStats(prev => ({ ...prev, ...statsRes.data }));
      } catch (err) { console.error('Stats error:', err); }

      // 2. Get pending approvals count
      try {
        const pendingRes = await axios.get(`http://${window.location.hostname}:5000/api/properties/pending-verification`);
      ...
      try {
        const pendingProfilesRes = await axios.get(`http://${window.location.hostname}:5000/api/profiles/pending`);
        setStats(prev => ({ ...prev, pendingProfiles: pendingProfilesRes.data.total }));
      } catch (err) { console.error('Profiles error:', err); }

      // 4. Get activities
      try {
        const activitiesRes = await axios.get(`http://${window.location.hostname}:5000/api/dashboard/activities`);
        setActivities(activitiesRes.data);
      } catch (err) { console.error('Activities error:', err); }

      // 5. Get suspicious properties count
      try {
        const suspRes = await axios.get(`http://${window.location.hostname}:5000/api/suspicious-activity/count`);
