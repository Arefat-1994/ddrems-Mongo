...
const UserSettingsEnhanced = ({ user, onLogout, onRefreshUser }) => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const prefsResponse = await axios.get(`${window.API_URL}/user-settings/${user.id}`);
      console.log('Settings loaded:', prefsResponse.data);
      setPreferences(prefsResponse.data);
      
      // Fetch 2FA settings
      try {
        const twoFAResponse = await axios.get(`${window.API_URL}/user-settings/${user.id}/two-factor`);
  ...
  const fetchActivityLogs = async () => {
    try {
      setLogsLoading(true);
      const response = await axios.get(`${window.API_URL}/user-settings/${user.id}/activity-logs`);
  ...
  const fetchActiveSessions = async () => {
    try {
      ...
      const response = await axios.get(`${window.API_URL}/user-settings/${user.id}/sessions`);
  ...
  const handleTerminateSession = async (sessionId) => {
    try {
      await axios.delete(`${window.API_URL}/user-settings/${user.id}/sessions/${sessionId}`);
      fetchActiveSessions();
  ...
  const handleTerminateAllOtherSessions = async () => {
    try {
      await axios.delete(`${window.API_URL}/user-settings/${user.id}/sessions`);
      fetchActiveSessions();
  ...
  const handleGlobalCloseSessions = async () => {
    try {
      await axios.post(`${window.API_URL}/system/sessions/close-all`, {}, { headers: { 'x-user-role': user.role } });
  ...
  const handleSave = async () => {
    try {
      ...
      // Save preferences
      await axios.post(`${window.API_URL}/user-settings/${user.id}`, preferences);
      
      // Save 2FA settings
      if (twoFactorSettings) {
        await axios.post(`${window.API_URL}/user-settings/${user.id}/two-factor`, twoFactorSettings);
      }
  ...
  const handlePhotoChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${window.API_URL}/users/upload-photo/${user.id}`, formData, {
        headers: {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      setSaving(true);
      await axios.post(`${window.API_URL}/auth/change-password`, {
        userId: user.id,
        currentPassword,
        newPassword
      });
                 ...
                 <div className="profile-details-section">
                   ...
                   <button 
                     ...
                     onClick={async () => {
                       ...
                       try {
                         setSaving(true);
                         await axios.put(`${window.API_URL}/users/${user.id}`, {
                           name: profileName
                         });
                    ...
                    <div className="action-buttons">
                      <button className="restart-btn" onClick={() => {
                        if(window.confirm('Are you sure you want to RESTART the system?')) {
                          axios.post(`${window.API_URL}/system/restart`, {}, { headers: { 'x-user-role': user.role } });
                        }
                      ...
                      <button className="shutdown-btn" onClick={() => {
                        if(window.confirm('Are you sure you want to SHUTDOWN the system? This will stop all services.')) {
                          axios.post(`${window.API_URL}/system/shutdown`, {}, { headers: { 'x-user-role': user.role } });
                        }
