...
const BrokerProfile = ({ user, onComplete }) => {
  ...
  const handleFileUpload = (e, field, setPreview) => {
    ...
    if (file.type.startsWith('image/')) {
      ...
      // For PDFs or non-images, just check size strictly to prevent MongoDB crash (Max 16MB per document)
      if (file.size > 2 * 1024 * 1024) {
        alert('PDFs and documents must be less than 2MB.');
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (profile) {
        await axios.put(`${window.API_URL}/profiles/broker/${profile.id}`, { ...formData, phone_number: fullPhoneNumber });
        ...
        await axios.post(`${window.API_URL}/profiles/broker`, {
          ...formData,
