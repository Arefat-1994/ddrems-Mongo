...
const InProgressView = ({ user, onLogout, setCurrentPage, onBack }) => {
  ...
  useEffect(() => {
    const fetchEngagements = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`${window.API_URL}/broker-engagement/broker/${user.id}`);
...
const AgentDashboardEnhanced = ({ user, onLogout, setCurrentPage, onSettingsClick }) => {
  ...
  const fetchAgentData = async () => {
    try {
      ...
      let activeCount = 0;
      try {
        const activeRes = await axios.get(`${window.API_URL}/properties/active`);
      ...
      try {
        const messagesRes = await axios.get(`${window.API_URL}/messages/user/${user.id}`);
        setMessages(messagesRes.data.slice(0, 5));
      } catch (error) { setMessages([]); }

      try {
        const announcementsRes = await axios.get(`${window.API_URL}/announcements`);
        setAnnouncements(announcementsRes.data.slice(0, 3));
      } catch (error) { setAnnouncements([]); }

      // 5. Fetch Agreements
      try {
        const agreementsRes = await axios.get(`${window.API_URL}/agreements/broker/${user.id}`);
