...
const LoginForm = ({ onLogin, onShowRegister, onBackToLanding }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${window.API_URL}/auth/login`, {
        email,
        password
      });
  ...
  const handleForgotSubmit = async (e) => {
    ...
    try {
      if (resetStep === 1) {
        // Request OTP
        const res = await axios.post(`${window.API_URL}/auth/forgot-password`, { email: resetEmail });
        setResetMessage(res.data.message);
        setResetStep(2);
      } else {
        // Verify OTP
        const res = await axios.post(`${window.API_URL}/auth/verify-otp`, { email: resetEmail, otp: otpCode });
        setResetMessage(res.data.message);
        setTimeout(() => {
