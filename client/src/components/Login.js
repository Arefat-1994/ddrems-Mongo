...
const LoginForm = ({ onLogin, onShowRegister, onBackToLanding }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/auth/login`, {
        email,
        password
      });
  ...
  const handleForgotSubmit = async (e) => {
    ...
    try {
      if (resetStep === 1) {
        // Request OTP
        const res = await axios.post(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/auth/forgot-password`, { email: resetEmail });
        setResetMessage(res.data.message);
        setResetStep(2);
      } else {
        // Verify OTP
        const res = await axios.post(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/auth/verify-otp`, { email: resetEmail, otp: otpCode });
        setResetMessage(res.data.message);
        setTimeout(() => {
