...
const NotificationComposer = ({ onClose, adminId }) => {
  ...
  const handleSearch = async (val) => {
    ...
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/users/search?q=${val}`);
      setUsers(res.data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/notifications`, {
      ...
      if (sendEmail && selectedUser.email) {
        await axios.post(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/system/send-custom-email`, {
