...
const Announcements = ({ user, onLogout, onSettingsClick }) => {
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/announcements`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/announcements/${editingId}`, formData);
      } else {
        await axios.post(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/announcements`, formData);
      }
      fetchAnnouncements();
      setShowModal(false);
      resetForm();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this announcement?')) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_URL || `http://${window.location.hostname}:5000`}/api/announcements/${id}`);
        fetchAnnouncements();
      } catch (error) {
