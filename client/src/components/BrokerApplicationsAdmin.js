...
const BrokerApplicationsAdmin = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://${window.location.hostname}:5000/api/broker-applications`);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      await axios.post(`http://${window.location.hostname}:5000/api/broker-applications/${id}/approve`);
  ...
  const handleReject = async (id) => {
    ...
    try {
      await axios.post(`http://${window.location.hostname}:5000/api/broker-applications/${id}/reject`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://${window.location.hostname}:5000/api/broker-applications/${id}`);
