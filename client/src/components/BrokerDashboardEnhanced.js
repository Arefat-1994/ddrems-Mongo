...
const BrokerDashboardEnhanced = ({ user, onLogout, setCurrentPage }) => {
  ...
  const fetchBrokerData = async () => {
    ...
    try {
      ...
      const profileRes = await axios.get(`${window.API_URL}/profiles/broker/${user.id}`);
      setBrokerProfile(profileRes.data);

      // Fetch incoming requests
      const agreementsRes = await axios.get(`${window.API_URL}/agreement-requests/broker/${user.id}`);
      ...
      // Fetch notifications
      const notifRes = await axios.get(`${window.API_URL}/notifications/user/${user.id}`);
      setNotifications(notifRes.data.slice(0, 5));

      // Fetch Commissions (with resilient error handling)
      try {
        const commRes = await axios.get(`${window.API_URL}/commissions/broker/${user.id}`);
        setCommissions(Array.isArray(commRes.data) ? commRes.data : []);
      } catch (commErr) { console.warn('Commission fetch failed:', commErr.message); }

      try {
        const summaryRes = await axios.get(`${window.API_URL}/commissions/broker/${user.id}/summary`);
      ...
      try {
        const engRes = await axios.get(`${window.API_URL}/broker-engagement/broker/${user.id}`);
        setBrokerEngagements(engRes.data.engagements || []);
      } catch (engErr) { console.warn('Engagements fetch failed:', engErr.message); }

      // Fetch Customers
      try {
        const custRes = await axios.get(`${window.API_URL}/broker-engagement/broker/${user.id}/customers`);
        setCustomers(custRes.data.customers || []);
      } catch (custErr) { console.warn('Customers fetch failed:', custErr.message); }

      // Fetch Broker Holds
      try {
        const holdsRes = await axios.get(`${window.API_URL}/broker-bookings?broker_id=${user.id}`);
  ...
  const handleAcceptRequest = async (requestId) => {
    try {
      await axios.put(`${window.API_URL}/agreement-requests/${requestId}/respond`, {
        status: 'accepted',
