...
const BrokerDashboardEnhanced = ({ user, onLogout, setCurrentPage }) => {
  ...
  const fetchBrokerData = async () => {
    ...
    try {
      ...
      const profileRes = await axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/profiles/broker/${user.id}`);
      setBrokerProfile(profileRes.data);

      // Fetch incoming requests
      const agreementsRes = await axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/agreement-requests/broker/${user.id}`);
      ...
      // Fetch notifications
      const notifRes = await axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/notifications/user/${user.id}`);
      setNotifications(notifRes.data.slice(0, 5));

      // Fetch Commissions (with resilient error handling)
      try {
        const commRes = await axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/commissions/broker/${user.id}`);
      ...
      // Fetch Customers
      try {
        const custRes = await axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/broker-engagement/broker/${user.id}/customers`);
        setCustomers(custRes.data.customers || []);
      } catch (custErr) { console.warn('Customers fetch failed:', custErr.message); }

      // Fetch Broker Holds
      try {
        const holdsRes = await axios.get(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/broker-bookings?broker_id=${user.id}`);
  ...
  const handleAcceptRequest = async (requestId) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_URL || (window.location.hostname.includes('vercel.app') ? 'https://ddrems-mongo.onrender.com' : `http://${window.location.hostname}:5000`)}/api/agreement-requests/${requestId}/respond`, {
        status: 'accepted',
