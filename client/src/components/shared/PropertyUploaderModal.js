...
const PropertyUploaderModal = ({ user, onClose, onSuccess }) => {
  ...
  const handleAddProperty = async (e) => {
    ...
    try {
      ...
      if (newPropertyId) {
        ...
        response = await axios.put(`${window.API_URL}/properties/${newPropertyId}`, payload);
      } else {
        // Create new property
        response = await axios.post(`${window.API_URL}/properties`, payload);
  ...
  const fetchPreviewData = async () => {
    try {
      const [propertyRes, imagesRes, docsRes] = await Promise.all([
        axios.get(`${window.API_URL}/properties/${newPropertyId}`),
        axios.get(`${window.API_URL}/property-images/property/${newPropertyId}`),
        axios.get(`${window.API_URL}/property-documents/property/${newPropertyId}`).catch(() => ({ data: [] }))
      ]);
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      await axios.put(`${window.API_URL}/properties/${newPropertyId}`, {
